...
def main():
    ...
    for x in MemberList:
        ...
        print('昵称:%s, 性别:%s, 备注:%s, 签名:%s' % (x['NickName'].encode('gbk','ignore').decode('gbk'),
                        sex.encode('gbk','ignore').decode('gbk'), x['RemarkName'].encode('gbk','ignore').decode('gbk'),
                                              x['Signature'].encode('gbk','ignore').decode('gbk')))
